...
interface FieldCheck { field: string; si_value: string | null; bl_value: string | null; match: boolean; }
interface EmailRecord {
  id: string; subject: string; sender: string;
  category: string; status: string | null; fields: FieldCheck[];
}
interface ShippingData { field: string; siValue: string; blValue: string; }
interface ReviewCase { id: string; relatedSubject: string; reason: string; status: string; }

const CATEGORY_LABELS: Record<string, string> = {
  BL_COMPARISON: 'Document Comparison', INVOICE_QUERY: 'Invoice Query',
  SI_REQUEST: 'New SI Request', GENERAL: 'General Message', SPAM: 'Spam',
};
...
const REASON_LABELS: Record<string, string> = {
  ...
  unreadable: 'Document unreadable', missing_value: 'Required value missing',
};
...
export default function Dashboard() {
  ...
  const [inbox, setInbox] = useState<EmailRecord[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setQueue(
          data
            .filter((r: any) => r.status === 'NEEDS_REVIEW')
            .map((r: any) => ({
              id: r.email_id,
  ...
  const comparisonData: ShippingData[] = selectedEmail
    ? selectedEmail.fields.map(f => ({
